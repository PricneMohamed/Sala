...
export default function Projectcontainer() {
  ...
  useEffect(()=>{
    const checkNextSala = setInterval(()=>{
      if(moment().isAfter(Fajr) &&
        moment().isBefore(Dhuhr)
      ){
        setnextSala("الظهر")
      }
      else if(moment().isAfter(Dhuhr) &&
        moment().isBefore(Asr)
      ){
        setnextSala("العصر")
      }
      else if(moment().isAfter(Asr) &&
        moment().isBefore(Maghrib)
      ){
        setnextSala("المغرب")
      }
      else if(moment().isAfter(Maghrib) &&
        moment().isBefore(Isha)
      ){
        setnextSala("العشاء")
      }
      else{
  ...
  return (
    <div className="Projectcontainer w-[100vw] h-[100vh]">
      <div className="text-white font-bold flex justify-evenly flex-wrap">
        ...
        <h1 className="text-[30px] mt-20 w-[250px]">{moment().format("MMM | Do | YY")}</h1>
      </div>
      <div className="text-white font-bold flex justify-evenly flex-wrap">
        <form className="max-w-sm w-[250px] mt-10">
          ...
          <select
            ...
            onChange={
              (e)=>{
               const cityname= e.target.value === "Alexandria"? chooseCity.city="الاسكندرية":chooseCity.city="القاهرة"
      ...
      <div className="Card flex justify-evenly flex-wrap gap-5 mt-32">
        <Card time={data.Fajr} name="الفجر" image={require("../img/_0350927d-7fda-441e-9c11-e4f4947f915c.jpeg")} />
        <Card time={data.Dhuhr} name="الظهر" image={require("../img/_16507fa3-8e68-4918-b0ea-c85f7b00e8b4.jpeg")} />
        <Card time={data.Asr} name="العصر" image={require("../img/_5005c191-07aa-489e-b47f-635a1ae1c664.jpeg")} />
        <Card time={data.Maghrib} name="المغرب" image={require("../img/_b7a97450-b59e-443d-a45e-a9630eabf713.jpeg")} />
        <Card time={data.Isha} name="العشاء" image={require("../img/_5601b845-b65a-4092-8d58-eb7b9cddf0c0.jpeg")} />
      </div>
    </div>
  );
}
